...
/** 進度 key：firered:<modelId>，與 funasr:<id> / qwen:<id> 同構，渲染層按前綴路由。 */
export function getFireRedProgressKey(id: FireRedModelId): string {
  return `firered:${id}`;
}
...
/**
 * FireRedASR 模型下載器：整包（tar.bz2）下載 + 解包，或 ModelScope 逐文件直下，
 * 落 userData/models/firered/<id>/。複用 downloadFileParallel（斷點續傳 + 多連接 + 取消），
 * 解包用 extractArchive（獨立進程 system tar）。與 QwenModelDownloader 同構。
 */
export class FireRedModelDownloader {
  ...
  async download(
    id: FireRedModelId,
    source: FireRedModelSource = FIRERED_DEFAULT_SOURCE,
  ): Promise<boolean> {
    if (isFireRedModelInstalled(id)) return true;
    const spec = FIRERED_MODELS[id];
    const key = getFireRedProgressKey(id);
    this.currentKey = key;
    ...
    for (const src of getFireRedSourceOrder(source)) {
      try {
        ...
        this.currentKey = null;
        logMessage(`firered model ${id} installed from ${src}`, 'info');
        return true;
      } catch (error) {
        lastError = error;
        ...
        logMessage(`firered model ${id} from ${src} failed: ${msg}`, 'warning');
      }
    }
   ...
   */
  private async downloadFromModelScope(spec: FireRedModelSpec): Promise<void> {
    const destDir = getFireRedModelDir(spec.id);

    let sizeByPath = new Map<string, number>();
    try {
      const tree = await fetchJson<{ Data?: { Files?: MsFileEntry[] } }>(
        getFireRedModelScopeTreeUrl(spec),
      );
      sizeByPath = new Map(
        (tree.Data?.Files ?? [])
      ...
      logMessage(
        `firered modelscope tree fetch failed: ${String(e)}`,
        'warning',
      );
    }
    ...
    for (const f of files) {
      ...
      const url = getFireRedModelScopeFileUrl(spec, f.remote);
      try {
  ...
  private async downloadFromArchive(
    ...
    const destDir = getFireRedModelDir(spec.id);
    const tmp = path.join(getFireRedModelsRoot(), spec.archiveName);
    const url = getFireRedArchiveUrl(spec, source);
...
let instance: FireRedModelDownloader | null = null;

export function getFireRedModelDownloader(
  mainWindow?: BrowserWindow,
): FireRedModelDownloader {
  if (!instance) instance = new FireRedModelDownloader(mainWindow);
